...
func NewServer(trinoClient *trino.Client, trinoConfig *config.TrinoConfig, version string) *Server {
	...
	if trinoConfig.OAuthEnabled {
		oauthHandler = CreateOAuth2Handler(trinoConfig, version)
	}
...
func NewHTTPServer(mcpServer *mcpserver.MCPServer, config *config.TrinoConfig, version string) *HTTPServer {
	...
	if config.OAuthEnabled {
		oauthHandler = CreateOAuth2Handler(config, version)
	}
...
func applyOAuthMiddleware(mcpServer *mcpserver.MCPServer, validator TokenValidator, enabled bool) error {
